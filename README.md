...
Java _virtual threads_ are lightweight threads designed to increase throughput in concurrent applications. Virtual threads were introduced as a preview feature in Java 19 (JEP 425) and Java 20 (JEP 436), and were officially finalized in Java 21 (JEP 444). Pre-existing Java threads were based on operating system (OS) threads that proved insufficient to meet the demands of modern concurrency. Servers nowadays must serve millions of concurrent requests, but the OS and thus the JVM cannot efficiently handle more than a few thousand threads.

Currently, programmers can either use threads as the units of concurrency and write synchronous blocking code in the thread-per-request model, or use asynchronous/reactive models that reuse threads without blocking them. The former applications are easier to develop, but they are not scalable because the number of OS threads is limited. The latter applications, while having much better scalability, are much more difficult to implement, debug, and understand.
...
For example, a CPU has 24 cores and the total request latency is W=100 ms. If a request spends W<sub>CPU</sub>=10 ms, then to fully utilize the CPU you need to have 240 threads. If a request requires much less computing resources and spends W<sub>CPU</sub>=0.1 ms, then to fully utilize the CPU you already need to have 24000 threads. However, a mainstream OS cannot support that number of active threads, mainly because the stack of each thread is too large. (A consumer-grade computer nowadays rarely supports more than 5000 active threads). Therefore, server computational resources are often underutilized when executing I/O bounded requests.
...
A summary of the quantitative differences between platform and virtual threads:
...
* starts a non-blocking I/O operation in the OS kernel
...
Thread.Builder builder = Thread.ofVirtual()
   ...
   .uncaughtExceptionHandler((t, e) -> System.out.printf("thread %s failed with exception %s%n", t, e));
...
[code examples](https://github.com/aliakh/demo-java-virtual-threads/blob/main/src/test/java/demo/virtualthreads/part2/README.md#write-blocking-synchronous-code-in-the-thread-per-task-style)
...
[code examples](https://github.com/aliakh/demo-java-virtual-threads/blob/main/src/test/java/demo/virtualthreads/part2/README.md#do-not-pool-virtual-threads)
...
[code examples](https://github.com/aliakh/demo-java-virtual-threads/blob/main/src/test/java/demo/virtualthreads/part2/README.md#use-semaphores-instead-of-fixed-thread-pools-to-limit-concurrency)
...
Sometimes, _scoped values_ may be a better alternative to thread-local variables. Unlike a thread-local variable, a scoped value is set once, is available only for a bounded context, and is inherited in a _structured concurrency_ scope.

[code examples](https://github.com/aliakh/demo-java-virtual-threads/blob/main/src/test/java/demo/virtualthreads/part2/README.md#use-thread-local-variables-carefully-or-switch-to-scoped-values)
...
[code examples](https://github.com/aliakh/demo-java-virtual-threads/blob/main/src/test/java/demo/virtualthreads/part2/README.md#use-synchronized-blocks-and-methods-carefully-or-switch-to-reentrant-locks)
